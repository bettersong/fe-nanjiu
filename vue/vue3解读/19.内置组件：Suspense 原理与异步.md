...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a44178ca30404637b8e775d4e5760271~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp) 上图中，红色字体代表的是组件有异步的 `setup()` 。 通过 `<Suspense>` 组件我们可以很容易实现在组件异步加载时统一展示加载中状态，在所有组件完成加载时，再统一展示：
...
```js
js
复制代码function mountSuspense(vnode, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized, rendererInternals) {
  const {
    p: patch,
    o: { createElement }
  } = rendererInternals
...
```js
js
复制代码function createSuspenseBoundary(vnode, parent, parentComponent, container, hiddenContainer, anchor, isSVG, slotScopeIds, optimized, rendererInternals, isHydrating = false) {
  // ...
  const suspense = {
    vnode,
    parent,
...
```js
js
复制代码const mountComponent = (initialVNode, container, anchor, parentComponent, parentSuspense, isSVG, optimized) => {
  // ...
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a44178ca30404637b8e775d4e5760271~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
```js
js
复制代码// 触发 onPending，onFallback 钩子函数
triggerEvent(vnode, 'onPending')
triggerEvent(vnode, 'onFallback')
...
```javascript
javascript
复制代码import { createApp, ref, h, onMounted } from 'vue'
...
```js
js
复制代码// queuePostRenderEffect 在 suspense 模式下指的是 queueEffectWithSuspense
...
```scss
scss
复制代码function patchSuspense(n1, n2, container, anchor, parentComponent, isSVG, slotScopeIds, optimized, { p: patch, um: unmount, o: { createElement } }) {
  // 初始化赋值操作
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d1d3dceb3cf84c509c458a291fb45fc6~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
