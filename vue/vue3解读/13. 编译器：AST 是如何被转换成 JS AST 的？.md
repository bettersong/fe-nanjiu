...
```js
js
复制代码function baseCompile(template, options) {
...
```js
js
复制代码return function render(_ctx) {
  with (_ctx) {
...
```js
js
复制代码import { toDisplayString, openBlock, createElementBlock } from "vue"
...
```js
js
复制代码function getBaseTransformPreset(prefixIdentifiers) {
  return [
    [
      transformOnce,
      transformIf,
...
```js
js
复制代码export function transform(root, options) {
  // 生成 transform 上下文
...
```javascript
javascript
复制代码export function createTransformContext(root, TransformOptions) {
  const context = {
    // 选项配置
    hoistStatic,
    cacheHandlers,
...
```js
js
复制代码export function traverseNode(node, context) {
...
```js
js
复制代码export const transformElement = (node, context) => {
  // 这里就是返回了一个退出函数
...
```js
js
复制代码function createVNodeCall(context, tag, props, children, patchFlag, dynamicProps, directives, isBlock = false, disableTracking = false, loc = locStub) {
  if (context) {
    if (isBlock) {
...
```js
js
复制代码export function hoistStatic(root, context) {
  walk(
    root,
    context,
    // 根节点是不可提升的
...
```js
js
复制代码function walk(node, context, doNotHoistNode = false) {
  const { children } = node
  // 记录那些被静态提升的节点数量
  let hoistedCount = 0
...
```js
js
复制代码import { createElementVNode as _createElementVNode, openBlock as _openBlock, createElementBlock as _createElementBlock } from "vue"
...
```php
php
复制代码const _hoisted_1 = /*#__PURE__*/_createElementVNode("p", null, null, -1 /* HOISTED */)
// ...
...
```js
js
复制代码const _hoisted_1 = /*#__PURE__*/_createStaticVNode("<p></p>...<p></p>", 20)
const _hoisted_21 = [
  _hoisted_1
]
...
```js
js
复制代码function createRootCodegen(root, context) {
...
```js
js
复制代码root.helpers = [...context.helpers.keys()]
root.components = [...context.components]
root.directives = [...context.directives]
root.imports = context.imports
root.hoists = context.hoists
