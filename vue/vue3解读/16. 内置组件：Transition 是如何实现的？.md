...
```js
js
复制代码export const Transition = (props, { slots }) => h(BaseTransition, resolveTransitionProps(props), slots)
...
```js
js
复制代码const leavingHooks = resolveTransitionHooks(
  oldInnerChild,
  rawProps,
  state,
  instance
...
```js
js
复制代码function resolveTransitionProps(rawProps) {
  const baseProps = {}
  // ...
...
```scss
scss
复制代码const makeEnterHook = (isAppear) => {
  return (el, done) => {
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e286d7a4e5294b2c995b164d9dc102e8~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
```js
js
复制代码export function resolveTransitionHooks(vnode, props, state, instance) {
  // 传入的各个钩子函数
  const {
    appear,
    mode,
