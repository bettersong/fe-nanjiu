...
计算属性接受一个 `getter` 函数，返回一个只读的响应式 [ref](https://link.juejin.cn/?target=https%3A%2F%2Fcn.vuejs.org%2Fapi%2Freactivity-core.html%23ref) 对象。该 `ref` 通过 `.value` 暴露 `getter` 函数的返回值。

```js
js
复制代码const count = ref(1)
...
```js
js
复制代码function computed(getterOrOptions, debugOptions, isSSR = false) {
  let getter
  let setter
  // 判断第一个参数是不是一个函数
...
```js
js
复制代码const { ref, effect, computed } = Vue
...
```js
js
复制代码function triggerEffects(dep, debuggerEventExtraInfo) {
