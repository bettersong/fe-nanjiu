...
![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8979e66c0c914abc9a97c92c228393fd~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
```js
js
复制代码import { createTextVNode as _createTextVNode, resolveComponent as _resolveComponent, withCtx as _withCtx, openBlock as _openBlock, createBlock as _createBlock } from "vue"
...
```js
js
复制代码function _createVNode(type, props, children, patchFlag, dynamicProps = null, isBlockNode = false) {
  // ...
  if (isVNode(type)) {
    // clone vnode
...
```js
js
复制代码export function normalizeChildren(vnode, children) {
  let type = 0
...
```js
js
复制代码export function setupComponent(instance) {
  // 1. 处理 props
...
```js
js
复制代码export const initSlots = (instance, children) => {
...
```js
js
复制代码import { renderSlot as _renderSlot, openBlock as _openBlock, createElementBlock as _createElementBlock } from "vue"
...
```js
js
复制代码export function renderSlot(slots, name, props, fallback, noSlotted) {
  // ...
  // 根据 name 获取 slot 内容
  let slot = slots[name]
  openBlock()
...
```js
js
复制代码export function withCtx(fn, ctx= currentRenderingInstance, isNonScopedSlot) {
  // ...
...
```js
js
复制代码const updateComponent = (n1, n2, vnode) => {
