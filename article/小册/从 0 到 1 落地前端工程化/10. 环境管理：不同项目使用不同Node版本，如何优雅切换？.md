...
鉴于这种情况，`Node Version Manager`应运而生，它不是一个工具，而是一系列可切换`Node版本`的工具总称，简称`Nvm`。`Nvm`在`Windows系统`中可选[nvm-windows](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fcoreybutler%2Fnvm-windows)，在`Mac系统`中可选[nvm-sh](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fnvm-sh%2Fnvm)。不要以为两个工具同源，其实它们无任何关系，但可执行的命令却一样。
...
打开`nvm-windows`的[下载地址](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fcoreybutler%2Fnvm-windows%2Freleases)，下载最新版本的`nvm-setup.zip`，解压后双击`nvm-setup.exe`文件，开启傻瓜式安装。
...
![Nvm路径](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/acd393c7e2694f0e822ba39aeed4600f~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)

![Node路径](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b37d100ed7f6446c9c56f44b938903bb~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![环境变量](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7dadf866300044e284f90283de98a529~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![多个版本](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/796933e2b64c4c45bbd654221d9bfce0~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)

> MacOS

打开`nvm-sh`的[下载地址](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fnvm-sh%2Fnvm%2Freleases)，确认最新版本。为了方便管理`Nvm`相关文件，创建`/Users/$USER/Documents/记录/Nvm`目录用于存放`Nvm`。若当前版本为`0.39.1`，执行以下命令安装`nvm-sh`。
...
打开`CMD工具`，登录服务器。执行`git --version`，输出版本表示软件存在。若未安装`git`，打开[Git](https://link.juejin.cn/?target=https%3A%2F%2Fgit-scm.com)查看其最新版本。若当前版本为`2.36.1`，通过以下方式安装`git`。
...
执行`git --version`，输出版本表示安装成功。打开`nvm-sh`的[下载地址](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fnvm-sh%2Fnvm%2Freleases)，确认最新版本。
...
可能有些同学会问，为何只选择`16.14.0`、`14.19.0`和`12.22.10`三个版本？那先从官网说起，打开[Node官网](https://link.juejin.cn/?target=https%3A%2F%2Fnodejs.org%2Fzh-cn)，发现官网推荐安装的两个版本分别是`Current最新尝鲜版`与`LTS长期维护版`。
...
![版本迭代](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ccce4ed971434134bd6e0a21ee6ca323~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)
...
`LTS`也不是万能的，为了稳步开发，选择最新版本`LTS`以及往下两个主版本共存，这样才能保障近段时间内项目运行的稳定性。若当前`TLS`为`16.14.0`，打开[版本列表](https://link.juejin.cn/?target=https%3A%2F%2Fnodejs.org%2Fzh-cn%2Fdownload%2Freleases)，查询`14.x`与`12.x`的最新版本。
